  ...
  let newPosition = pointMe (board g) c p
   in let updatedMaze = removeCompass (board g) c
       in let newPlayerInfo = updatePoints p (playerInfo g) c
           in S.put (S.execState (whichMove newPosition p) (g {playerInfo = newPlayerInfo, board = updatedMaze}))
...
-- >>> player1 (playerInfo (S.execState (collectCompass Cell {x = 6, y = 5, isWall = False} One) testGame))
-- Attributes {numPoints = 1, position = Cell {x = 6, y = 5, isWall = False}}
